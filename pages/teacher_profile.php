...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (!$errors) {
        ...
        // Handle avatar upload (teachers folder)
        $avatarPath = $user['profile_image'];
        $uploadBase = dirname(__DIR__) . '/uploads/teachers';
        ...
        if (isset($_FILES['avatar']) && !empty($_FILES['avatar']['name'])) {
            if (is_uploaded_file($_FILES['avatar']['tmp_name'])) {
                ...
                if (!in_array($ext, $allowed, true) || ($mime && !in_array($mime, $allowedM, true))) {
                    ...
                    $newName = 'avatar_teacher_' . $userId . '_' . date('YmdHis') . '_' . bin2hex(random_bytes(4)) . '.' . $ext;
        ...
        if (!$errors) {
            try {
                ...
                logAction($userId, 'update_profile', 'Teacher updated profile.');
...
// Build avatar URL similar to header handling
$avatarUrl = '';
if (!empty($user['profile_image'])) {
    ...
    if (preg_match('/^https?:\/\//i', $pi)) {
        ...
        $p = ltrim($pi, '/');
        if (strpos($p, 'rinda/') === 0) { $p = substr($p, strlen('rinda/')); }
        if (strpos($p, 'uploads/') === 0) {
            $avatarUrl = '../' . $p;
        } else {
            $avatarUrl = '../uploads/teachers/' . rawurlencode(basename($p));
        }
    } else {
        $avatarUrl = '../uploads/teachers/' . rawurlencode($pi);
    }
}
    ...
    <div class="card">
      ...
      <form method="POST" enctype="multipart/form-data" class="form">
          ...
          <div>
            ...
            <div style="margin:8px 0;">
              <?php if (!empty($avatarUrl)): ?>
                <img src="<?php echo htmlspecialchars($avatarUrl); ?>" alt="Avatar" style="width:96px;height:96px;object-fit:cover;border-radius:50%;border:1px solid var(--border-color);">
              <?php else: ?>
        ...
        <div style="margin-top:12px">
          ...
          <a class="btn btn-secondary" href="./teacher_change_password.php"><i class="fas fa-key"></i> Change Password</a>
        </div>
      </form>
      </div>
    </div>
