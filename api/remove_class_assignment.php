...
try {
    ...
    // Remove assignment
    $stmt = $pdo->prepare("DELETE FROM teacher_class_assignments WHERE teacher_id = ? AND class_id = ?");
    $stmt->execute([$teacher_id, $class_id]);
    
    // Log the action
    $log_stmt = $pdo->prepare("INSERT INTO system_logs (user_id, action, description, additional_data) VALUES (?, 'class_unassigned', 'Removed class assignment from teacher', ?)");
