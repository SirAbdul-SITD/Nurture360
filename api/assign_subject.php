...
try {
    ...
    if ($check_stmt->fetch()) {
        ...
        echo json_encode(['error' => 'Teacher is already assigned to this subject']);
        exit;
    }
    
    // Create assignment
    $stmt = $pdo->prepare("INSERT INTO teacher_subject_assignments (teacher_id, subject_id, assigned_at) VALUES (?, ?, NOW())");
    $stmt->execute([$teacher_id, $subject_id]);
    
    // Log the action
    $log_stmt = $pdo->prepare("INSERT INTO system_logs (user_id, action, description, additional_data) VALUES (?, 'subject_assigned', 'Assigned subject to teacher', ?)");
